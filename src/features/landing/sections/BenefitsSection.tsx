...
import { ArrowDown01, ArrowUp01, Maximize2, X, BarChart2, Loader2 } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription, DialogFooter } from '@/components/ui/dialog';
import type { BenefitsSectionProps, IndustryData, YearOption } from '../types/benefit.types';
...
export function BenefitsSection({ content, onSave }: BenefitsSectionProps) {
  ...
  const [industries] = useState<string[]>([
    'Tecnología', 'Finanzas', 'Manufactura', 'Comercio',
    'Servicios', 'Construcción', 'Agricultura', 'Energía'
  ]);
  ...
  useEffect(() => {
    if (selectedIndustry && selectedYear) {
      ...
      setTimeout(() => {
        ...
        setSelectedIndustryData(data.find(d => d.industry === selectedIndustry) || null);
        setLoading(false);
      }, 500);
    }
  ...
  const IndustrySelect = ({ hidden }: { hidden?: boolean }) => (
    ...
    >
      <option value="" hidden>{hidden ? 'AÑO' : 'SELECCIONE UNA INDUSTRIA'}</option>
  ...
  const YearSelect = ({ shortLabel }: { shortLabel?: boolean }) => (
    ...
    >
      <option value="" hidden>{shortLabel ? 'AÑO' : 'SELECCIONE AÑO'}</option>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-full w-screen h-screen rounded-none flex flex-col p-0 gap-0">
          ...
          <div className="flex-1 p-6 overflow-y-auto relative">
            ...
            <div ref={chartModalRef} className="w-full h-[620px]">
              {!selectedIndustry || !selectedYear
                ? <EmptyChart />
