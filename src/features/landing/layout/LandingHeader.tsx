    ...
    <header className="relative w-full" id="home">
      <nav className={`
        ...
        <div className="max-w-[1400px] mx-auto px-6 grid grid-cols-2 lg:grid-cols-12 items-center h-full">
          ...
          {/* 1. SECCIÓN IZQUIERDA: Logo o Mobile Toggle  */}
          ...
          {/* 2. SECCIÓN CENTRAL: Menu Nav (Desktop) */}
          ...
          {/* 3. SECCIÓN DERECHA: Soporte (Contactanos y Footer) - Login/UserMenu */}
      ...
      <div className={`fixed inset-0 z-[100] lg:hidden transition-all duration-300 ${isMobileMenuOpen ? 'visible' : 'invisible'}`}>
        <div onClick={() => setIsMobileMenuOpen(false)} className={`absolute inset-0 bg-black/40 backdrop-blur-sm 
          transition-opacity duration-300 ${isMobileMenuOpen ? 'opacity-100' : 'opacity-0'}`}
        />

        {/* SideBar Izquierda Menú */}
        <aside className={` absolute top-0 left-0 h-full w-[280px] bg-white shadow-2xl 
