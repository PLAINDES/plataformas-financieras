...
function reportToForm(report: Report): ReportFormData {
  return {
    ...
    portadaNombre: report.portada?.nombre ?? "",
  };
}
...
export const ReporteKapitalEditor: React.FC = () => {
  ...
  //Descomentar cuando se tenga el archivo de covers
  //En espera a integración con S3
  // const portadaAlt = report?.portada?.portada?.alt_text ?? "Portada";

  const isChartOrTable = (tc: TemplateCodeBasic) => {
    const text = `${tc.nombre} ${tc.code}`.toLowerCase();
    return text.includes("grafico") || text.includes("gráfico") || text.includes("tabla");
  };
  ...
  const handleSave = async () => {
    ...
    try {
      await MainService.updateReport(Number(id), {
        ...
        activo: form.activo,
        ...(form.portadaId !== (report?.portada?.id ?? null) && {
          cover_data: { portada_id: form.portadaId },
        }),
      });
        ...
        <div className="flex flex-col gap-6 p-6 lg:flex-row">
          ...
          <div className="flex flex-1 flex-col gap-5 min-w-0">
            ...
            <div className="rounded-xl border border-slate-100 bg-slate-50/60 p-5">
              <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-end">
                ...
                <div className="flex items-center gap-2 pb-1">
                  ...
                  />
                  <Label htmlFor="activo" className="cursor-pointer text-xs font-semibold text-slate-600">
                    Activar
                  </Label>
                </div>
              </div>
              ...
              <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
                {(
                  ...
                  <div key={fieldId}>
                    <Label htmlFor={fieldId} className="mb-1.5 block text-xs font-semibold text-slate-600 uppercase">
                      {label}
                    </Label>
                    <Input
                      id={fieldId}
              ...
              <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end">
                <div className="w-full sm:w-1/4">
                  <Label htmlFor="contenido" className="mb-1.5 block text-xs font-semibold text-slate-600 uppercase">
                    Contenido
                  </Label>
                  <Input
                    id="contenido"
                ...
                <div className="flex-1">
                  <Label htmlFor="linkPago" className="mb-1.5 block text-xs font-semibold text-slate-600 uppercase">
                    Link de pago
                  </Label>
                  <Input
                    id="linkPago"
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
                <div className="flex-1">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => handleChange("portadaId", v ? Number(v) : null)}
                  >
                  ...
                  */}
                  {portadaUrl ? (
                    ...
                    <div className="text-center">
                      ...
                      <p className="mt-0.5 text-[8px] font-bold leading-tight text-blue-100 uppercase">KAPITAL</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="flex justify-end gap-2">
              <Button
                variant="outline" size="sm" className="text-sm h-9 px-4"
                onClick={() => navigate("/admin/kapital/reportes")}
              >
                Cancelar
              </Button>
