...
export interface ValoraResultsProps {
  formData: FormData;
  section: "estados" | "resultados" | "analisis" | "metodologia";
...
export const ValoraResults: React.FC<ValoraResultsProps> = ({
  ...
  const [longTermGrowthRate, setLongTermGrowthRate] = useState(1.5);
  const [capitalCostRate, setCapitalCostRate] = useState(12.43);
  const [incomeGrowthRate, setIncomeGrowthRate] = useState(0.0);
  ...
  const conceptosData = [
    {
      ...
      headerText: "MÉTODO POR CONCEPTOS",
      tooltipText:
        "Mediante este metodo, VALORA realiza la proyeccion de cada componente del Flujo de Caja Economico (FCE).",
      iconClassName:
        "fa-solid fa-triangle-exclamation fs-1 text-[#a1a5b7] text-2xl",
      patrimonioValue: 0,
      empresaValue: 0,
      accionValue: 0,
    ...
    },
    {
      ...
      headerText: "MÉTODO INTEGRADO",
      tooltipText:
        "Mediante este método, VALORA realiza una proyección del flujo de caja operativo de forma histórica, complementándola con la proyección de las inversiones y la variación en el capital de trabajo para obtener el FCE proyectado.",
      iconClassName: "fa-solid fa-chart-pie fs-1 text-[#a1a5b7] text-2xl",
      patrimonioValue: 0,
      empresaValue: 0,
      accionValue: 0,
  ...
  const tooltipText =
    "Defina la tasa de crecimiento de los ingresos para el primer año de proyección. La tasa por defecto es la recomendada por la plataforma.";
