...
const BetaUpdateCard: React.FC<BetaUpdateCardProps> = ({
  response,
  newBeta,
  onUpdate,
}) => (
  <div className="space-y-3">
    ...
    <div className="rounded-xl border-l-4 border-sky-400 bg-linear-to-r from-sky-50 to-slate-50 p-3 flex items-center justify-between gap-3">
      ...
      <button
        ...
        className="shrink-0 text-xs font-semibold text-white bg-linear-to-r from-sky-400 to-blue-600 px-4 py-2 rounded-lg transition-all hover:-translate-y-0.5 hover:shadow-lg active:translate-y-0"
      >
        Actualizar Beta
      </button>
    </div>
...
const MessageBubble: React.FC<MessageBubbleProps> = ({ msg }) => {
  ...
  return (
    <div
      className={`flex gap-2 ${isUser ? "flex-row-reverse" : ""} mb-4`}
      style={{ animation: "fadeUp 0.3s ease" }}
    >
      <div
        className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 text-white text-sm ${isUser ? "bg-slate-500" : "bg-sky-500"}`}
      >
        {isUser ? (
      ...
      <div
        className={`flex flex-col max-w-[88%] ${isUser ? "items-end" : "items-start"}`}
      >
        <div
          className={`px-4 py-2.5 rounded-2xl text-[13px] leading-relaxed shadow-sm whitespace-pre-wrap wrap-break-words ${
            isUser
              ? "bg-linear-to-br from-sky-400 to-blue-600 text-white rounded-br-sm"
...
export const Chatbot: React.FC<ChatbotProps> = ({ geminiApiKey = "" }) => {
  ...
  const updateFinancialData = (financialData: FinancialData) => {
    ...
    if (financialData.dc_ratio != null)
      fire("dcRatioFormInput", financialData.dc_ratio.toFixed(4));
    if (financialData.effective_tax_rate != null)
      fire(
        "effectiveTaxRateFormInput",
        (financialData.effective_tax_rate * 100).toFixed(2)
      );
    if (financialData.beta_levered != null)
      fire("betaLeveredFormInput", financialData.beta_levered.toFixed(4));
    if (financialData.beta_unlevered != null)
      fire("betaUnleveredFormInput", financialData.beta_unlevered.toFixed(4));
    setTimeout(
  ...
  const analyzeYahooTickers = useCallback(
    async (tickers: string[]) => {
      ...
      try {
        ...
        addSimple(
          `❌ Error analizando empresas: ${(e as Error).message}`,
          "ai"
        );
      } finally {
        setLoading(false);
    ...
    async (userMessage: string) => {
      ...
      try {
        ...
        if (!key || !key.startsWith("AIza")) {
          addSimple(
            "API key no configurada o inválida. Contacta al administrador.",
            "ai"
          );
          setLoading(false);
          return;
...
- Para empresas internacionales usa sufijos (.TO, .L, .PA)`;
          ...
          ...(history.length === 0
            ? [
                { role: "user", parts: [{ text: systemPrompt }] },
                {
                  ...
                  parts: [
                    {
                      text: "Perfecto. Soy tu asistente especializado en análisis de beta para WACC. ¿Quieres que analice tus datos actuales?",
                    },
                  ],
                },
              ]
        ...
        if (tickersMatch) {
          const tickers = tickersMatch[1]
            .split(",")
            .map((t) => t.trim())
            .filter(Boolean);
          const displayText = formatAPIResponse(
            rawResponse.replace(/TICKERS:\[([^\]]+)\]/i, "").trim()
          );
          ...
          if (betaMatch) {
            const cleanText = formatAPIResponse(
              rawResponse.replace(/BETA_UPDATE:\s*[\d.]+/i, "").trim()
            );
            pushItem({
              id: uid(),
              type: "beta",
              betaData: {
                response: cleanText,
                newBeta: parseFloat(betaMatch[1]),
              },
              time: now(),
            });
          } else {
  ...
  return (
    <>
      ...
      <div className="fixed bottom-5 right-5 z-1050 flex flex-col items-end">
        {isOpen && (
          <div
            className="mb-4 w-115 max-w-[calc(100vw-40px)] rounded-2xl shadow-2xl overflow-hidden border border-slate-200 bg-white"
            style={{
            ...
            <div className="px-4 py-3 bg-slate-50 border-t border-slate-200">
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  style={{
                    background:
                      "linear-gradient(135deg,#009ef7 0%,#007dc4 100%)",
                  }}
                >
                  {loading ? (
                    <svg
