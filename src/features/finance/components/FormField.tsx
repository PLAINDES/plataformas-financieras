...
export const FormField: React.FC<FormFieldProps> = ({
    ...
    return (
        <div className="grid grid-cols-1 gap-2 md:grid-cols-12 md:items-center">
            {
                label != '' &&
                <label className="text-sm text-gray-600 md:col-span-4 font-semibold">{label}</label>
            }
            <div className="md:col-span-8 bg-white">
                {type === 'select' ? (
                    <div ref={containerRef} className="relative">
                        ...
                        {required && (
                            <input type="text" name={name} value={value} readOnly required className="hidden" />
                        )}
                        {isOpen && (
                    ...
                    <div className="relative">
                        <input
                            type="text"
                            className="w-full rounded border border-gray-300 px-3 py-2 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500"
                            value={value}
                            name={name}
                            onChange={onChange}
                            required={required}
